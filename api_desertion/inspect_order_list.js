$(function(){
	...
	//初始化验房订单
	function initInspectPage(page,curpage){
		$.ajax({
			type:'post',
			url:ApiUrl+"/index.php?act=member_inspect_order&op=list&page="+page+"&curpage="+curpage,	
			data:{key:key},
			dataType:'json',
			success:function(result){
				...
				var html = template.render('inspect-order-list-tmpl', data);
				$("#inspect-order-list").html(html);
	...
	function sureOrder(){
		...
		$.ajax({
			type:"post",
			url:ApiUrl+"/index.php?act=member_inspect_order&op=payComplete",
