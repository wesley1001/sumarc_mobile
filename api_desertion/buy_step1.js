$(function () {
    ...
    if (ifcart == 1) {
        cart_id = GetQueryString('cart_id');
    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            $.each(data.store_cart_list, function (k, v) {
                //循环店铺
                if (i == 0) {
                    htmldata += '<li>';
                } else {
                ...
                $.each(v.goods_list, function (k1, v1) {
                    ...
                    htmldata += '<div class="cfix">'
                    + '<input type="checkbox" onclick="points()" name="shipping_type" class="shipping_type" value="' + v1.goods_id + '" title="自提" style="height:60px; float:left; display:block; margin-top:8px;" />'
                ...
                /** @namespace v.store_mansong_rule_list */
                /** @namespace v.store_goods_total */
                /** @namespace v.store_mansong_rule_list.discount */
                /** @namespace data.store_cart_list */

                htmldata += '<div class="shop-total">' +
                '<div class="total-price">' +
                '<span>运费：￥</span>';
                var sp_total;
                if (v.store_mansong_rule_list != null) {
                    sp_total = eval(v.store_goods_total - v.store_mansong_rule_list.discount);
                    htmldata += '<p class="clr-c07">本店合计：￥<span id="st' + k + '" store_price="' + sp_total + '" class="store_total">' + sp_total + '</span></p>';
                } else {
                    sp_total = v.store_goods_total;
                    htmldata += '<p class="clr-c07">本店合计：￥<span id="st' + k + '" store_price="' + sp_total + '" class="store_total">' + sp_total + '</span></p>';
                }

                htmldata += '</div>';
                //增加送货时间
                htmldata += '<div class="delivery-time">';
                htmldata += '<p>送货时间 : <input type="text" name="deliver_time[' + k + ']" class="deliver_time" id="deliver_time' + k + 'id" store_id="' + k + '" value="" /></p>';
                htmldata += '<p>请选择' + v.deliver_time + '后的日期</p>'
                htmldata += '</div></li>';
                total_price = eval(sp_total + total_price);
            });

            $("#deposit").append(htmldata);//订单列表

            //var tomorrow = document.createTextNode(moment().add(7, 'days').format('YYYY MM DD') + ' ');
            //document.getElementById('selected').appendChild(tomorrow);

            $.each(data.store_cart_list, function (k, v) {
                var deliver_time = 'deliver_time' + k + 'id';

                var picker = new Pikaday({
                    field: document.getElementById(deliver_time),
                    firstDay: 1,
                    ...
                    onSelect: (function () {
                        $.ajax({
                            type: 'post',
                            url: ApiUrl + '/index.php?act=member_index&op=check_deliver_time&inajax=1',
                            data: 'deliver_time=' + $('#'+deliver_time).val() + '&store_id=' + $('#'+deliver_time).attr('store_id') + "&key=" + key,
                            success: function (ret) {
                                if (ret) {
                                    alert('对不起,你不能选择提前发货时间');
                                    $('#'+deliver_time).val(ret);
                                }
                            }
                        });
                    }),
    ...
    $("select[name=region]").change(function () {
        ...
        $.ajax({
            ...
            success: function (result) {
                ...
                $("select[name=town]").html(region_html);
            }
        });
    });
    ...
    $('#buy_step2').click(function () {
        ...
        $(".deliver_time").each(function () {
            var store_id = $(this).attr('store_id');
            deliver_time[store_id] = $(this).val();
        });
        data.deliver_time = deliver_time;

        var shipping_type = new Array();
        var ii = 0;
        $('input[name="shipping_type"]:checked').each(function () {
            shipping_type[ii] = $(this).val();
            ii++;
        });
